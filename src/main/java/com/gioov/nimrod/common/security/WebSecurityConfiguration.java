...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        // 添加验证码校验过滤器
        http.addFilterBefore(verifyCodeFilter, UsernamePasswordAuthenticationFilter.class).authorizeRequests().antMatchers(Api.System.VERIFY_CODE).permitAll();
        ...
        http.csrf().disable()
                ...
                .rememberMe().tokenValiditySeconds(60 * 60 * 60 * 24 * 7).rememberMeParameter(Page.User.LOGIN_REMEMBER_ME_STRING).and()
                // 注销
                .logout().logoutUrl(Api.User.LOGOUT).logoutSuccessHandler(logoutSuccessHandler).deleteCookies("JSESSIONID", "remember-me");

        // 一个帐号只允许同时在线一个 session
        http.sessionManagement().sessionAuthenticationFailureHandler(authenticationFailureHandler).maximumSessions(1).expiredUrl(Page.User.LOGIN);
