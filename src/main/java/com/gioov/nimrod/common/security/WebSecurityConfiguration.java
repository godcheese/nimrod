...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        // 添加验证码校验过滤器
        http.addFilterBefore(verifyCodeFilter, UsernamePasswordAuthenticationFilter.class).authorizeRequests().antMatchers(Url.Api.System.VERIFY_CODE).permitAll();
        ...
        http.csrf().disable()
                ...
                // 登录页，无需登录认证权限直接访问
                .antMatchers(Url.Page.User.LOGIN).permitAll()
                // 其它请求均需要权限认证
                ...
                // 自定义登录表单提交 url
                .loginProcessingUrl(Url.Api.User.LOGIN)
                ...
                .rememberMe().tokenValiditySeconds(60 * 60 * 60 * 24 * 7).rememberMeParameter(Url.Page.User.LOGIN_REMEMBER_ME_STRING).and()
                // 注销
                .logout().logoutUrl(Url.Api.User.LOGOUT).logoutSuccessHandler(logoutSuccessHandler).deleteCookies("JSESSIONID", "remember-me");

        // 一个帐号只允许同时在线一个 session
        http.sessionManagement().sessionAuthenticationFailureHandler(authenticationFailureHandler).maximumSessions(1).expiredUrl(Url.Page.User.LOGIN);
